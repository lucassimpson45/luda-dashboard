...
import type { CampaignForm, OnboardFormState } from '@/lib/admin-onboard-types'
import {
  BUSINESS_TYPES,
  US_TIMEZONES,
  buildStepFlow,
...
function CampaignSection({
  ...
  return (
    <div className="space-y-4 rounded-xl border border-neutral-200 p-4 dark:border-neutral-700">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className={labelClass}>Follow-up interval (hours)</label>
          <input
            type="number"
            min={1}
            className={inputClass}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange({ ...value, maxAttempts: Math.max(1, Number(e.target.value) || 1) })}
          />
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {value.steps.map((step, i) => (
          ...
          >
            ...
            <div>
              <label className={labelClass}>Delay (hours from previous)</label>
              <input
                type="number"
                min={0}
                className={inputClass}
